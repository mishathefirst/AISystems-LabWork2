...
public class BidirectionalSearch {
    ...
    Stack<Node> visitedFrontNodes = new Stack<>();
    Stack<Node> visitedBackNodes = new Stack<>();

    public void runBidirectionalSearch() {
        ...
        visitedFrontNodes.push(simferopol);
        System.out.println("Starting from " + simferopol.getName());
        visitedBackNodes.push(murmansk);
        System.out.println("Starting from " + murmansk.getName());
    ...
    public void runFrontIteration() {
        for (Node node : visitedFrontNodes.peek().getConnections().keySet()) {
            if (visitedBackNodes.search(node.getName()) == -1) {
                if ((!node.isClosed()) && (visitedFrontNodes.search(node) == -1)) {
                    visitedFrontNodes.push(node);
                    System.out.println("Front method entered " + node.getName());
                    node.setClosed(true);
                    runBackIteration();
                    System.out.println("Front method left " + node.getName());
                    node.setClosed(false);
                }
            } else {
                System.out.println("Path found!");
                visitedFrontNodes.push(node);
                for (Node movedNode : visitedBackNodes) {
                    if (!movedNode.getName().equals(node.getName())) {
                        visitedBackNodes.pop();
                    } else {
                        break;
                    }
                }
                for (Node movedNode : visitedBackNodes) {
                    visitedFrontNodes.push(visitedBackNodes.pop());
                }
                System.out.println("The final path:");
                for (Node finalNode : visitedFrontNodes) {
        ...
        visitedFrontNodes.pop().setClosed(true);
    }




    public void runBackIteration() {
        for (Node node : visitedBackNodes.peek().getConnections().keySet()) {
            if (visitedFrontNodes.search(node.getName()) == -1) {
                if ((!node.isClosed()) && (visitedBackNodes.search(node) == -1)) {
                    visitedBackNodes.push(node);
                    System.out.println("Back method entered " + node.getName());
                    node.setClosed(true);
                    runFrontIteration();
                    System.out.println("Back method left " + node.getName());
                    node.setClosed(false);
                }
            } else {
                System.out.println("Path found!");
                visitedBackNodes.push(node);
                for (Node movedNode : visitedFrontNodes) {
                    if (!movedNode.getName().equals(node.getName())) {
                        visitedFrontNodes.pop();
                    } else {
                        break;
                    }
                }
        ...
        visitedBackNodes.pop().setClosed(true);
    }
