...
public class DFSIterativeDeeping {
    ...
    List<Node> listOfCities = Arrays.asList(simferopol, spb, vilnius, brest, vitebsk, voronezh, volgograd,
            nizhniyNovgorod, daugavpils, kaliningrad, kaunas, kiev, zhitomir, donetsk, kishinev, riga, kazan,
            moscow, minsk, murmansk, orel, odessa, tallinn, kharkov, yaroslavl, ufa, samara);
    private int depth = 0;
    private int counterOfDepth = 0;


    public void runDFSWithIterativeDeeping() {
        ...
        /*
        for (Node node : listOfCities) {
            depth += node.getConnections().size();
        }
         */

        //depth = depth / 2 + 1;
        //System.out.println("Greatest possible depth value of the graph is " + depth);
    ...
    private void runIteration() {
        ...
        /*
        if (counterOfDepth > depth) {
            System.out.println("Greatest possible depth overflow. The graph does not have an appropriate path ");
            System.out.println("or program runs an infinite loop in the decision tree.");
            System.exit(0);
        }
         */
        for (Node node : visitedNodes.peek().getConnections().keySet()) {
            if (!node.getName().equals("Murmansk")) {
                ...
                System.out.println("Entered Murmansk.");
                counterOfDepth++;
                System.out.println("Path found!");
                System.out.println("Depth of the decision tree is " + counterOfDepth);
                System.exit(0);
            }
